...
def test(vanilla_lstm_net,args,pred_len=0):
    ...
    for i, batch in enumerate(dataloader):
        if(args.use_cuda): #when use_cuda has been mentioned in the argument
        ...
        if(args.use_cuda): #to transfer the tensors back to CPU if cuda was used
            out1=out1.cpu()
...
#     lstm_net.eval() # set dropout and batch normalization layers to evaluation mode before running inference
    ...
    for i in range(num_epoch):
        ...
        avgTestLoss,avgD_test,finalD_test,num_test_peds=test(vanilla_lstm_net,args,pred_len)
